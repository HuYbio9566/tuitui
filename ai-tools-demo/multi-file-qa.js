...
function layout(content) {
  return `
    <div class="app-shell qa-shell">
      <aside class="sidebar">
        <div class="brand"><span class="brand-mark">360</span><span>AI 云盘工具</span></div>
        <div class="global-search"><i data-lucide="search"></i><span>搜索工具、文件或报告</span></div>
        <div class="side-actions"><button class="side-action">上传</button><button class="side-action">新建</button></div>
        <div class="side-label">工具导航</div>
        <nav class="side-nav">
          <a class="side-link" href="index.html"><i data-lucide="layout-grid"></i>新增工具</a>
          <a class="side-link active" href="multi-file-qa.html"><i data-lucide="messages-square"></i>多文件问答</a>
          <a class="side-link" href="existing-tools.html"><i data-lucide="archive"></i>已有工具</a>
          <a class="side-link" href="report.html"><i data-lucide="file-text"></i>调研报告</a>
        </nav>
        <div class="side-spacer"></div>
        <div class="account"><span class="account-avatar">许</span><span>许老师 · 市场部</span></div>
      </aside>
      <main class="main">
        <header class="topbar">
  ...
  return `
    <section class="qa-entry">
      <div class="qa-entry-copy">
        <p class="qa-eyebrow">证据约束对话工作台</p>
        <h1>和一组文件对话，每个结论都能回到原文</h1>
        <p>动态限定资料范围、核对引用，并把可信回答整理成对比表、简报或 FAQ。</p>
      </div>
      <div class="qa-entry-layout">
        <aside class="collection-panel">
          <div class="panel-title"><span>来源集合</span><button class="icon-btn" data-action="open-picker" aria-label="添加来源"><i data-lucide="plus"></i></button></div>
          <div class="collection-name"><i data-lucide="folder-kanban"></i><div><strong>春季招生决策资料</strong><small>${state.selectedSourceIds.length} 个来源 · 今天更新</small></div></div>
          <div class="entry-source-list">
            ${state.selectedSourceIds.map(id => renderEntrySource(sourceById(id))).join("")}
          </div>
          <button class="btn wide" data-action="open-picker"><i data-lucide="folder-plus"></i>从云盘添加来源</button>
          <div class="collection-safety"><i data-lucide="shield-check"></i><span>仅使用你有权限的内容，回答会标注实际来源。</span></div>
        </aside>
        <main class="question-start">
          <div class="start-card">
            <div class="start-icon"><i data-lucide="messages-square"></i></div>
            <h2>想从这些资料中了解什么？</h2>
            <p>已选择 ${state.selectedSourceIds.length} 个来源，可随时调整问答范围。</p>
            <div class="start-composer">
              <textarea id="entry-question" placeholder="例如：比较各渠道投入、到课表现和建议动作"></textarea>
              <div class="composer-foot"><span><i data-lucide="paperclip"></i>仅检索已选来源</span><button class="send-btn" data-action="start-question" aria-label="开始提问"><i data-lucide="arrow-up"></i></button></div>
            </div>
          </div>
          <section class="preset-section">
            <div class="section-heading"><h2>常用任务</h2><span>自动选择适合的回答结构</span></div>
            <div class="preset-grid">
              ${PRESETS.map(item => `<button class="preset-card" data-action="start-preset" data-preset="${item.id}"><i data-lucide="${item.icon}"></i><strong>${item.title}</strong><span>${item.prompt}</span></button>`).join("")}
            </div>
          </section>
          <section class="recent-questions">
            <div class="section-heading"><h2>最近问答</h2><button class="text-btn" data-action="start-workspace">查看全部</button></div>
            <button class="recent-question" data-action="start-workspace"><span><i data-lucide="message-circle"></i>上周预算调整最终以哪份资料为准？</span><small>3 个来源 · 6 条引用 · 昨天 18:42</small></button>
            <button class="recent-question" data-action="start-workspace"><span><i data-lucide="message-circle"></i>试听课反馈中家长最关注什么？</span><small>2 个来源 · 4 条引用 · 07-11</small></button>
          </section>
        </main>
      </div>
    </section>
...
function renderEntrySource(source) {
  return `<div class="entry-source"><span class="file-badge ${source.color}">${source.type.slice(0, 1)}</span><div><strong>${source.name}</strong><small>${source.type} · ${source.pages} ${source.type === "Excel" ? "个工作表" : "页"}</small></div><span class="source-ready"><i data-lucide="check"></i></span></div>`;
}
  ...
  return `
    <section class="qa-workspace-page">
      <div class="workspace-heading">
        <div><p class="qa-eyebrow">多文件问答工作区</p><h1>春季招生决策资料</h1><p>${state.selectedSourceIds.length} 个来源参与本轮问答 · 回答仅基于勾选资料</p></div>
      ...
      <div class="qa-workspace" data-mobile-panel="${state.mobilePanel}">
        ${renderSourceRail()}
        ${renderConversation()}
        ${renderEvidencePanel()}
      </div>
...
function renderSourceRail() {
  return `
    <aside class="source-rail qa-pane-source">
      <div class="pane-head"><div><h2>来源</h2><span>${state.selectedSourceIds.length}/${SOURCES.length} 已选择</span></div><button class="icon-btn" data-action="open-picker" aria-label="添加来源"><i data-lucide="plus"></i></button></div>
      <div class="scope-note"><i data-lucide="scan-search"></i><span>取消勾选后，后续回答不再使用该文件。</span></div>
      <div class="workspace-source-list">
        ${SOURCES.map(source => `
          <label class="workspace-source ${state.selectedSourceIds.includes(source.id) ? "selected" : ""}">
            <input type="checkbox" data-source-toggle="${source.id}" ${state.selectedSourceIds.includes(source.id) ? "checked" : ""}>
            <span class="file-badge ${source.color}">${source.type.slice(0, 1)}</span>
            <span><strong>${source.name}</strong><small>${source.type} · 已解析 · ${source.updated}</small></span>
          </label>
        `).join("")}
      </div>
      <button class="btn wide" data-action="open-picker"><i data-lucide="folder-plus"></i>添加云盘文件</button>
      <div class="source-footer"><span><i data-lucide="lock-keyhole"></i>权限变化会在回答前重新校验</span></div>
    </aside>
  `;
}
  ...
  return `
    <main class="conversation qa-pane-chat">
      <div class="conversation-toolbar">
        <div><strong>新问答</strong><span>模型：企业知识问答</span></div>
        <div class="answer-tools">
          ...
          <button data-action="transform" data-transform="faq"><i data-lucide="list-help"></i>FAQ</button>
        </div>
      </div>
      <div class="message-stream">
        ${state.messages.length ? state.messages.map(renderMessage).join("") : renderEmptyConversation()}
      </div>
      <div class="chat-composer">
        <textarea id="workspace-question" placeholder="继续追问，或要求比较、提取、核对……"></textarea>
        <div class="composer-foot"><span><i data-lucide="files"></i>本轮将检索 ${state.selectedSourceIds.length} 个来源</span><button class="send-btn" data-action="send-question"><i data-lucide="arrow-up"></i></button></div>
      </div>
    </main>
  `;
}
...
function renderEmptyConversation() {
  return `<div class="conversation-empty"><div class="empty-orbit"><i data-lucide="quote"></i></div><h2>从有出处的回答开始</h2><p>AI 会先检索已选来源，再组织答案；引用可在右侧核对原文。</p><div class="suggestion-row">${PRESETS.slice(0, 3).map(item => `<button data-action="ask-suggestion" data-kind="${item.id}">${item.prompt}</button>`).join("")}</div></div>`;
}

function renderMessage(message) {
  if (message.role === "user") return `<article class="message user-message"><div class="message-avatar">许</div><div><span class="message-label">你</span><p>${message.text}</p></div></article>`;
  const answer = message.answer;
  return `
    <article class="message assistant-message ${answer.status}">
      <div class="ai-avatar">AI</div>
      <div class="answer-body">
        <div class="answer-heading"><div><span class="message-label">基于 ${answer.usedSourceIds.length} 个来源</span><h2>${answer.title}</h2></div>${answer.status === "conflict" ? `<span class="conflict-badge"><i data-lucide="triangle-alert"></i>需人工确认</span>` : answer.status === "insufficient" ? `<span class="insufficient-badge">资料不足</span>` : `<span class="grounded-badge"><i data-lucide="shield-check"></i>来源已核对</span>`}</div>
        ...
        ${answer.citations.length ? `<div class="citation-strip"><span>引用</span>${answer.citations.map((item, index) => `<button class="citation-chip ${state.activeCitationId === item.id ? "active" : ""}" data-action="open-citation" data-citation="${item.id}">${index + 1} · ${sourceById(item.sourceId).name}</button>`).join("")}</div>` : ""}
        <div class="message-actions"><button data-action="toast" data-toast="回答已复制"><i data-lucide="copy"></i>复制</button><button data-action="transform" data-transform="brief"><i data-lucide="file-plus-2"></i>转为产物</button><button data-action="toast" data-toast="已提交反馈"><i data-lucide="thumbs-up"></i></button></div>
      </div>
    </article>
  `;
}
...
function renderEvidencePanel() {
  const item = CITATIONS[state.activeCitationId] || availableCitations(state.selectedSourceIds)[0];
  if (!item || !state.selectedSourceIds.includes(item.sourceId)) {
    return `<aside class="evidence-rail qa-pane-evidence"><div class="pane-head"><div><h2>来源证据</h2><span>等待选择引用</span></div></div><div class="evidence-empty"><i data-lucide="mouse-pointer-click"></i><p>点击回答中的引用，在这里核对原文和上下文。</p></div></aside>`;
  }
  const source = sourceById(item.sourceId);
  return `
    <aside class="evidence-rail qa-pane-evidence">
      <div class="pane-head"><div><h2>来源证据</h2><span>回答引用定位</span></div><button class="icon-btn" data-action="toast" data-toast="已在云盘打开原文件"><i data-lucide="external-link"></i></button></div>
      <div class="evidence-file"><span class="file-badge ${source.color}">${source.type.slice(0, 1)}</span><div><strong>${source.name}</strong><small>${item.place}</small></div></div>
      <div class="confidence-row"><span>引用置信度</span><strong>${item.confidence}%</strong></div>
      <div class="confidence-track"><span style="width:${item.confidence}%"></span></div>
      <div class="source-preview">
        <div class="preview-toolbar"><span>${item.heading}</span><div><button data-action="toast" data-toast="已定位上一处"><i data-lucide="chevron-up"></i></button><button data-action="toast" data-toast="已定位下一处"><i data-lucide="chevron-down"></i></button></div></div>
        <p>${item.before}</p>
        <blockquote>${item.excerpt}</blockquote>
        <p>${item.after}</p>
      </div>
      <div class="evidence-meta"><div><span>云盘位置</span><strong>${source.path}</strong></div><div><span>更新时间</span><strong>${source.updated}</strong></div></div>
      <button class="btn wide" data-action="toast" data-toast="已打开原文件并定位到 ${item.place}"><i data-lucide="locate-fixed"></i>打开原文并定位</button>
    </aside>
  `;
}
